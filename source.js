...
btn.addEventListener("click", async () => {
    try {
        ...
        const response = await fetch(`https://api.nationalize.io/?name=${name_value.value}`);
        ...
        if (countries_codes.length > 0) {
            ...
            for (let cc of countries_codes) {
                ...
                const response = await fetch(`https://restcountries.com/v2/alpha/${cc.country_id}`);
                const data = await response.json();
            ...
            document.querySelector(".flex-wrapper").innerHTML = "nott found result";
        }
    } catch (error) {
        console.log(error);
    }
